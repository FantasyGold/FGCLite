...
export function showModal(width: number, height: number, style: any = {}, src?: string): Promise<HTMLIFrameElement> {
  ...
  addClass(iframe, 'fgclite-extension-modal');

  const backdrop = document.createElement('div');
  addClass(backdrop, 'fgclite-extension-modal-backdrop');

  const html = document.documentElement;
  addClass(html, 'fgclite-extension-modal-wrap');
...
export function closeModal() {
  const iframe = document.querySelector('iframe.fgclite-extension-modal');

  if (iframe) {
    iframe.remove();
    const backdrop = document.querySelector('.fgclite-extension-modal-backdrop');

    if (backdrop) { backdrop.remove(); }

    removeClass(document.documentElement, 'fgclite-extension-modal-wrap');
  }
}
