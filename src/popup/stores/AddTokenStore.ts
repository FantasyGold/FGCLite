...
const INIT_VALUES = {
  ...
  symbol: '',
  decimals: undefined,
  getFGCTokenDetailsFailed: false,
};
...
export default class AddTokenStore {
  ...
  @observable public getFGCTokenDetailsFailed?: boolean = INIT_VALUES.getFGCTokenDetailsFailed;
  @computed public get contractAddressFieldError(): string | undefined {
    return (!!this.contractAddress
      && isValidContractAddressLength(this.contractAddress)
      && !this.getFGCTokenDetailsFailed)
      ...
      () => {
        ...
        if (this.contractAddress && !this.contractAddressFieldError) {
          chrome.runtime.sendMessage(
            { type: MESSAGE_TYPE.GET_FGC_TOKEN_DETAILS,
  ...
  private resetTokenDetails = () => {
    ...
    this.getFGCTokenDetailsFailed = INIT_VALUES.getFGCTokenDetailsFailed;
  }

  @action
  private handleMessage = (request: any) => {
    switch (request.type) {
      case MESSAGE_TYPE.FGC_TOKEN_DETAILS_RETURN:
        if (request.isValid) {
          ...
          this.getFGCTokenDetailsFailed = true;
        }
        break;
      default:
        break;
