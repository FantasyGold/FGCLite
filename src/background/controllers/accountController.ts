...
import { Wallet as FantasyGoldWallet } from 'fantasygoldjs-wallet';
import assert from 'assert';

import FGCLiteController from '.';
import IController from './iController';
import { MESSAGE_TYPE, STORAGE, NETWORK_NAMES, FGCLITE_ACCOUNT_CHANGE } from '../../constants';
...
export default class AccountController extends IController {
  ...
  */
  public addAccountAndLogin = async (accountName: string, privateKeyHash: string, wallet: FantasyGoldWallet) => {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `fgclite_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public onAccountLoggedIn = async (isSessionRestore = false) => {
     ...
     * reopening the popup, we don't need to send the SEND_INPAGE_FGCLITE_ACCOUNT_VALUES event to
     * the inpage because window.fgclite.account has not changed.
     */
    if (!isSessionRestore) {
      this.main.inpageAccount.sendInpageAccountAllPorts(FGCLITE_ACCOUNT_CHANGE.LOGIN);
    }
  ...
  */
  private recoverFromPrivateKeyHash(privateKeyHash: string): FantasyGoldWallet {
  ...
  private getPrivateKeyHash(wallet: FantasyGoldWallet) {
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    ...
    if (infoDidUpdate) {
      ...
      if (sendInpageUpdate) {
        this.main.inpageAccount.sendInpageAccountAllPorts(FGCLITE_ACCOUNT_CHANGE.BALANCE_CHANGE);
      }

      this.updateAndSendMaxFantasyGoldAmountToPopup();
    }
  }
  ...
  */
  private sendTokens = async (receiverAddress: string, amount: number, transactionSpeed: TRANSACTION_SPEED) => {
    ...
    */
    try {
      ...
      const feeRate = rates[transactionSpeed]; // satoshi/byte; 500 satoshi/byte == .005 FGC/KB
      if (!feeRate) {
   ...
   * For instance when a user sends FantasyGold, but that transaction has not confirmed yet,
   * the balance will not change, but calcMaxFantasyGoldSend is able to account for those
  ...
  private updateAndSendMaxFantasyGoldAmountToPopup = async () => {
    ...
    const calcMQSPr = this.loggedInAccount.wallet.calcMaxFantasyGoldSend(this.main.network.networkName);
    calcMQSPr.then(() => {
      chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_MAX_FGC_SEND_RETURN,
        maxFantasyGoldAmount: this.loggedInAccount!.wallet!.maxFantasyGoldSend });
    });
  }
    ...
    try {
      switch (request.type) {
        ...
        case MESSAGE_TYPE.GET_WALLET_INFO:
          ...
          break;
        case MESSAGE_TYPE.GET_FGC_USD:
          sendResponse(this.loggedInAccount && this.loggedInAccount.wallet
            ? this.loggedInAccount.wallet.fantasygoldUSD : undefined);
          break;
        case MESSAGE_TYPE.VALIDATE_WALLET_NAME:
          sendResponse(this.isWalletNameTaken(request.name));
          break;
        case MESSAGE_TYPE.GET_MAX_FGC_SEND:
          this.updateAndSendMaxFantasyGoldAmountToPopup();
          break;
        default:
          break;
