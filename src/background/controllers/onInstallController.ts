...
export default class OnInstallController extends IController {
  constructor(main: FGCLiteController) {
    ...
    * dapp apps using FGCLite. We want to refresh these tabs so that they can
    ...
    * injected scripts from prior versions of FGCLite.
    * To get refreshed, dapp tabs must implement the
    * handleFGCLiteInstallOrUpdate event listener described in the Readme.
    */
  ...
  private refreshTab(tab: chrome.tabs.Tab) {
     // Tells the content script to post a msg to the inpage window letting it know that FGCLite was installed or updated.
     chrome.tabs.executeScript(tab.id!, {code:
      `window.postMessage(
        {
          message: { type: 'FGCLITE_INSTALLED_OR_UPDATED' }
        },
        '*'
      )`,
    });
